...
class StockStatement(Document):
	...
	def upend_trigger(self):
		...
		tot_ok, tot_cr, tot_mr,tot_acr,tot_rw = 0, 0, 0, 0, 0
		...
		self.balance = (self.opening_stock + self.total_inward) - (tot_ok + tot_cr + tot_mr + tot_acr + tot_rw)
	...
	def op_stock(self):
		...
		opening_balance = frappe.db.sql("""
				...
				LIMIT 1
				""".format(self.from_date, "Stores - USE", self.item_code, "Unitech Steel Engineering", False), as_dict=True)
		if opening_balance:
